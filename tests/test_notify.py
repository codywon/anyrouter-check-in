...
def test_push_message(
	mock_telegram, mock_server_push, mock_feishu, mock_pushplus, mock_wecom, mock_dingtalk, mock_email, notification_kit
):
	"""测试 push_message 默认行为（包含邮件）"""
...
@patch('utils.notify.NotificationKit.send_serverPush')
@patch('utils.notify.NotificationKit.send_telegram')
def test_push_message_skip_email(
	mock_telegram, mock_server_push, mock_feishu, mock_pushplus, mock_wecom, mock_dingtalk, mock_email, notification_kit
):
	"""测试 push_message 跳过邮件通知"""
	notification_kit.push_message('测试标题', '测试内容', skip_email=True)

	assert not mock_email.called  # 邮件不应该被调用
	assert mock_dingtalk.called
	assert mock_wecom.called
	assert mock_pushplus.called
	assert mock_feishu.called
